...
fn load_edge_file(path: &Path) -> Result<(Vec<(u32, u32, f64)>, usize), String> {
    ...
    for line in content.lines() {
        ...
        let u: u32 = parts[0].parse().map_err(|_| format!("Invalid node: {}", parts[0]))?;
        let v: u32 = parts[1].parse().map_err(|_| format!("Invalid node: {}", parts[1]))?;
...
fn test_network(name: &str, edge_file: &str) {
    ...
    match std::time::Instant::now().checked_duration_since(std::time::Instant::now()) {
        Some(_) => {}
        None => {}
    }
    ...
    match detector.detect(&graph) {
        Ok(partition) => {
            ...
            let mut unique = membership.iter().copied().collect::<Vec<_>>();
